...
function TvDetail({ data, category }: any) {
    ...
    const { data: detailData, isLoading: detailLoading } =
        useQuery<IMovieDetail>(
            ['movies', `detail_${bigTvMatch?.params.movieId}`],
            () => getDetail(bigTvMatch?.params.movieId),
        );
    console.log(detailData?.genres[0].name);
    ...
    const clickedMovie =
        ...
        data?.results.find((movie: IMovie) => {
            return String(movie.id) == bigTvMatch.params.movieId;
        });
